    ...
    return Base.extend("cod3rsgrowth.webapp.controller.TelaCriar", {
        ...
        _aoAdiquirirClube: async function(evento){
            this._exibirEspera(async () => {
                ...
                .then((resposta) => {
                    oView.setModel(new JSONModel(resposta) , NOME_MODELO_CLUBE);
                })
                .catch(() => {
        ...
        _salvarModelo: function(){
            const modelo = this._modelo(NOME_MODELO_CLUBE).getData();
        ...
        _criarOuEditarClube: async function(hash, dadosDaCriacao) {
            if (hash[INDICEUM_NO_ARAY] == NOME_ROTA_EDITAR) {
                let idClube = this._modelo(NOME_MODELO_CLUBE).getData().id;
